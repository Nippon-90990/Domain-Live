...
//   }
//   let products = await Product.find({category: 'Sticker'}) //  // if you not 
//   return {
  ...
  return (
    <div>
      ...
      <section className="text-gray-600 body-font">
        <div className="container px-5 py-24 mx-auto">
          <div className="flex flex-wrap -m-4 justify-center">
            {Object.keys(products).length === 0 && <p className='text-orange-700 font-extrabold text-xl text-center'> Sorry Sir . This Item's Are Out of Stock...</p>}
...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({ category: 'Sticker' }) // Enter Your Category Name Like Tshirt/tshirt/T-shirt  // In My Case This Is T - Shirt
  let Stickers = {}
  for (let item of products) {
    if (item.title in Stickers) {
      if (!Stickers[item.title].color.includes(item.color) && item.availablQty > 0) {
        Stickers[item.title].color.push(item.color)
      }
      if (!Stickers[item.title].size.includes(item.size) && item.availablQty > 0) {
